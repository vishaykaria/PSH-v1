          ...
          <div className={s.exportSection}>
            ...
            <div>
              <FormGroup controlId="formControlsSelect" className={s.noMargin}>
                <FormControl onChange={(e) => this.handleDropDown(e)} value={userType} componentClass="select" placeholder={formatMessage(messages.selectLabel)} className={cx(bt.commonControlSelect)}>
                ...
                userManagement && userManagement.usersData.length > 0 && userManagement.usersData.map((value, key) => {
                  ...
                  return (
                    <Tr key={key}>
                      ...
                      <Td data-label={formatMessage(messages.createdDate)} column={formatMessage(messages.createdDate)} data={moment(value.profile && value.profile.createdAt).format('DD/MM/YYYY')} className={s.userVerticalAlign} />
