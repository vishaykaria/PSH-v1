...
const getPayouts = {
    ...
    async resolve({ request }, { currentAccountId, userId }) {
        if (request.user && !request.user.admin) {
            ...
            let isVerified = true, accountInfo = null, defaultValue = false, methodArray = [];

            if (payEmail && payEmail.toString().trim() != '') {
                const stripeAccount = await stripe.accounts.retrieve(payEmail);
                let accountUserId = stripeAccount && stripeAccount.metadata && stripeAccount.metadata.userId;
